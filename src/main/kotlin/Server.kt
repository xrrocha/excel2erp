...
data class Server(
    ...
    private val workbookBuilders = mapOf<String, (InputStream) -> Workbook>(
            "xls" to ::HSSFWorkbook,
            "xlsx" to ::XSSFWorkbook,
    )

    private fun openWorkBook(uploadedFile: UploadedFile): Workbook =
        workbookBuilders[extension(uploadedFile.filename())]?.let {
            it(
                    uploadedFile.content()
            )
        }
            ?: throw IllegalArgumentException("No workbook type for '${uploadedFile.filename()}'")
